...
class LCAOMatrices:
    ...
    def custom_S_blocks(self, k_list):
        ...
        S_blocks = np.zeros((self.n_blocks, self.m_max, self.m_max), dtype='complex')
...
class LCAOAtomIntegrals:
    ...
    def get_atom_func(self):
        ...
        xmax_single = 40 # Extent of the grid
        lam = 5
        xg = np.linspace(0, xmax_single, N_single)
        h = xg[1] - xg[0]
        V = poeschl_teller(xg, lam=lam, a=self.scale_H)
    ...
    def create_potential(self):
        x_u, V_u = make_potential_unitcell(
            # lambda x:-5/((x*10)**2+ 1),
            lambda x : poeschl_teller(x, lam=5, a=self.scale_H), 
            n_points=self.n_points, a=self.a
        )
...
if __name__ == "__main__":
    matrices = LCAOMatrices(a=14, n_points=2000, num_k=1000, m_max=4, scale_H=0.2, cached_int=False) #good parameters: a=2.2, 2.3; 2.4; , scale_H=0.7; => 1.08 eV;1.5 eV; 4eV
